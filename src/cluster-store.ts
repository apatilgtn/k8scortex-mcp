 ...
 */
function buildLocalClient(): CachedClient {
 ...
 */
async function fetchKeyVaultSecret(secretName: string): Promise<string> {
  if (!KEYVAULT_URI) {
 ...
 */
function buildClientFromBase64Kubeconfig(b64kubeconfig: string): CachedClient {
 ...
 */
export async function getK8sClientForCluster(clusterName: string = "default"): Promise<CachedClient> {

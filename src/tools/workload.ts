...
export async function handleWorkloadTool(name: string, args: any) {
  ...
  try {
    switch (name) {
      case "list_pods": {
        ...
        const pods = res.items.map((p: any) => {
      ...
      case "get_pod_logs": {
        ...
        try {
          ...
          return { content: [{ type: "text", text: res as unknown as string }] };
        } catch (err: any) {
      ...
      case "describe_deployment": {
        ...
        const summary = {
          ...
          containers: res.spec?.template?.spec?.containers?.map((c: any) => ({
            name: c.name,
            image: c.image,
            requests: c.resources?.requests || {},
            limits: c.resources?.limits || {},
      ...
      case "list_nodes": {
        ...
        const nodes = res.items.map((n: any) => ({
      ...
      case "list_statefulsets": {
        ...
        const statefulSets = res.items.map((s: any) => ({
      ...
      case "list_daemonsets": {
        ...
        const daemonSets = res.items.map((d: any) => ({

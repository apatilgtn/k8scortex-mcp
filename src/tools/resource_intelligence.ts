...
function parseCpu(cpuString: string): number {
  ...
  if (cpuString.endsWith('m')) return parseInt(cpuString.slice(0, -1), 10);
  if (cpuString.endsWith('n')) return parseInt(cpuString.slice(0, -1), 10) / 1000000;
  return parseFloat(cpuString) * 1000; // Return in millicores
}

function parseMemory(memString: string): number {
  if (!memString) return 0;
  if (memString.endsWith('Ki')) return parseInt(memString.slice(0, -2), 10) * 1024;
  if (memString.endsWith('Mi')) return parseInt(memString.slice(0, -2), 10) * 1024 * 1024;
  if (memString.endsWith('Gi')) return parseInt(memString.slice(0, -2), 10) * 1024 * 1024 * 1024;
  if (memString.endsWith('m')) return parseInt(memString.slice(0, -1), 10) / 1000;
...
function isMetricsApiUnavailable(error: any): boolean {
...
export async function handleResourceIntelligenceTool(name: string, args: any): Promise<any> {
  ...
  try {
    switch (name) {
        ...
        if (deployment.spec?.template?.spec?.containers) {
          for (const container of deployment.spec.template.spec.containers) {
            if (container.name === args.containerName) {
              ...
              if (args.cpuRequest) container.resources.requests["cpu"] = args.cpuRequest;
              if (args.memoryRequest) container.resources.requests["memory"] = args.memoryRequest;
              if (args.cpuLimit) container.resources.limits["cpu"] = args.cpuLimit;
              if (args.memoryLimit) container.resources.limits["memory"] = args.memoryLimit;
            }
          }
        }

        if (!found && (deployment.spec?.template?.spec?.containers?.length || 0) === 1) {
          const container = deployment.spec!.template!.spec!.containers![0] as any;
          if (!container.resources) container.resources = {};
          if (!container.resources.requests) container.resources.requests = {};
          if (!container.resources.limits) container.resources.limits = {};
      ...
      case "get_resource_recommendations": {
        ...
        if (!selector) throw new Error("Deployment has no selector");
        ...
        if (depRes.spec?.template?.spec?.containers) {
          for (const container of depRes.spec.template.spec.containers) {
            ...
            for (const podMetric of (podMetrics as any).items) {
              const containerMetric = podMetric.containers.find((c: any) => c.name === container.name);
              if (containerMetric) {
                const cpu = parseCpu(containerMetric.usage.cpu);
                const mem = parseMemory(containerMetric.usage.memory);
                if (cpu > maxCpuUsage) maxCpuUsage = cpu;
                if (mem > maxMemUsage) maxMemUsage = mem;
              }
            }
      ...
      case "get_cluster_resource_utilisation": {
        ...
        const utilisation = nodesRes.items.map((node: any) => {
          const name = node.metadata.name;
          const metric = (nodeMetrics as any).items.find((m: any) => m.metadata.name === name);

...
export async function handleDeploymentTool(name: string, args: any) {
  ...
  try {
    switch (name) {
      ...
      case "rollout_status": {
        ...
        const availableCondition = dep.status?.conditions?.find((c: any) => c.type === "Available");
      ...
      case "rollout_undo": {
        ...
        const owned = (rsList.items || []).filter((rs: any) =>
          (rs.metadata?.ownerReferences || []).some((o: any) => o.kind === "Deployment" && o.name === deploymentName)
        );
        ...
        const parsed = owned
          ...
          .filter((x: any) => Number.isFinite(x.revision) && x.revision > 0)

...
export async function handleClusterAdminTool(name: string, args: any): Promise<any> {
  ...
  try {
    switch (name) {
      ...
      case "drain_node": {
        ...
        const podsToEvict = podsRes.items.filter((pod: any) => {
          // Filter out DaemonSet pods
          const ownerReferences = pod.metadata?.ownerReferences || [];
          return !ownerReferences.some((ref: any) => ref.kind === "DaemonSet");
        });
      ...
      case "remove_taint": {
        ...
        const filtered = existing.filter((t: any) => t.key !== args.key);

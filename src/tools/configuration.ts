...
export async function handleConfigurationTool(name: string, args: any) {
  ...
  try {
    switch (name) {
      ...
      case "describe_namespace_quota": {
        ...
        const quotas = res.items.map((q: any) => ({
      ...
      case "list_persistent_volume_claims": {
        ...
        const claims = res.items.map((pvc: any) => ({

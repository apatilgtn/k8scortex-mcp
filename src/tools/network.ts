import { CoreV1Api, NetworkingV1Api, loadYaml } from "@kubernetes/client-node";
  ...
  try {
    switch (name) {
      ...
      case "list_ingresses": {
        ...
        const ingresses = res.items.map((i: any) => ({
          name: i.metadata?.name,
          namespace: i.metadata?.namespace,
          className: i.spec?.ingressClassName || null,
          hosts: (i.spec?.rules || []).map((r: any) => r.host).filter(Boolean),
          addresses: (i.status?.loadBalancer?.ingress || []).map((a: any) => a.ip || a.hostname).filter(Boolean),
        }));
      ...
      case "get_service_endpoints": {
        ...
        const summary = {
          ...
          namespace,
          readyAddresses: sets.flatMap((s: any) => (s.addresses || []).map((a: any) => ({
            ip: a.ip,
            nodeName: a.nodeName || null,
            targetRef: a.targetRef ? `${a.targetRef.kind}/${a.targetRef.name}` : null,
          }))),
          notReadyAddresses: sets.flatMap((s: any) => (s.notReadyAddresses || []).map((a: any) => ({
            ip: a.ip,
            nodeName: a.nodeName || null,
            targetRef: a.targetRef ? `${a.targetRef.kind}/${a.targetRef.name}` : null,
          }))),
          ports: sets.flatMap((s: any) => (s.ports || []).map((p: any) => ({
            name: p.name || null,
            port: p.port,
            protocol: p.protocol,
          }))),

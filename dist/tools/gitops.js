...
// ── Helpers ──────────────────────────────────────────────────────────────────
async function fetchGitOpsApp(customObjectsApi, engine, namespace, appName) {
    if (engine === "argocd") {
...
function fluxReadyCondition(conditions) {
    return conditions?.find((c) => c.type === "Ready") || {};
}
async function patchFluxSuspend(customObjectsApi, resourceType, namespace, name, suspend) {
    const { group, version, plural } = fluxGroupConfig(resourceType);
    const resource = await customObjectsApi.getNamespacedCustomObject({ group, version, namespace, plural, name });
...
function fluxGroupConfig(resourceType) {
...
async function listFluxResources(customObjectsApi, group, version, plural, namespace) {
    if (namespace) {
        const res = await customObjectsApi.listNamespacedCustomObject({ group, version, namespace, plural });
        return res.items || [];
    }
    const res = await customObjectsApi.listClusterCustomObject({ group, version, plural });
    return res.items || [];
}
// ── Handler ───────────────────────────────────────────────────────────────────
export async function handleGitOpsTool(name, args) {
    try {
        switch (name) {
            ...
            case "get_gitops_diff": {
                ...
                let diffSummary = {};
                if (args.engine === "argocd") {
                    const outOfSync = (app.status?.resources || []).filter((r) => r.status !== "Synced");
                    diffSummary = {
                    ...
                    const inventory = (app.status?.inventory?.entries || []).map((e) => {
            ...
            case "compare_clusters": {
                const fetchForCluster = async (clusterName) => {
                    try {
                ...
                const summary = (app, clusterName) => {
                    if (app.error)
                        return app;
                    if (args.engine === "argocd")
                        return { revision: app.status?.sync?.revision, syncStatus: app.status?.sync?.status };
            ...
            case "get_flux_helm_release": {
                const cluster = args.cluster || "default";
                const { customObjectsApi } = await getK8sClientForCluster(cluster);
                const hr = await customObjectsApi.getNamespacedCustomObject({
                ...
                const detail = {
                    ...
                    conditions: (hr.status?.conditions || []).map((c) => ({
            ...
            case "list_flux_alerts": {
                ...
                const summary = items.map((a) => ({
                    ...
                    eventSources: (a.spec?.eventSources || []).map((s) => `${s.kind}/${s.name}`),

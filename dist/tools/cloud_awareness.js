...
function detectNodePoolLabel(nodes) {
    const candidates = [
...
function isMetricsUnavailable(err) {
...
function detectWorkloadIdentityCloud(annotations = {}) {
    if (annotations["azure.workload.identity/client-id"] || annotations["azure.workload.identity/tenant-id"])
        return "aks";
...
export async function handleCloudAwarenessTool(name, args) {
        ...
        switch (name) {
            case "list_node_pools": {
                ...
                const output = Array.from(pools.entries()).map(([poolName, poolNodes]) => {
                    const ready = poolNodes.filter((n) => n.status?.conditions?.some((c) => c.type === "Ready" && c.status === "True")).length;
            ...
            case "get_node_pool_detail": {
                ...
                let metricsByNode = new Map();
                try {
                    const metrics = await customObjectsApi.listClusterCustomObject({ group: "metrics.k8s.io", version: "v1beta1", plural: "nodes" });
                    for (const m of metrics.items || [])
                        metricsByNode.set(m.metadata?.name, m.usage || {});
                }
                catch (e) {
            ...
            case "get_workload_identity_config": {
                ...
                const cloud = detectWorkloadIdentityCloud(annotations);
                const config = {
                    namespace,
                    serviceAccountName,
                    cloud,
            ...
            case "validate_workload_identity": {
                ...
                const podChecks = (pods.items || []).map((p) => ({
                    pod: p.metadata?.name,
            ...
            case "list_pod_disruption_budgets": {
                const namespace = args.namespace;
                const pdbs = await customObjectsApi.listNamespacedCustomObject({ group: "policy", version: "v1", namespace, plural: "poddisruptionbudgets" });
                const items = (pdbs.items || []).map((p) => ({
            ...
            case "get_pdb_status": {
                ...
                const pdb = await customObjectsApi.getNamespacedCustomObject({ group: "policy", version: "v1", namespace, plural: "poddisruptionbudgets", name: pdbName });
                const selector = pdb.spec?.selector?.matchLabels || {};
                const labelSelector = Object.entries(selector).map(([k, v]) => `${k}=${v}`).join(",");
                const pods = labelSelector ? await coreV1Api.listNamespacedPod({ namespace, labelSelector }) : { items: [] };
                return {
                    content: [{
                            type: "text",
                            text: safeJson({
                                ...
                                coveredPods: (pods.items || []).map((p) => ({ name: p.metadata?.name, phase: p.status?.phase })),
                            }),
                        }],
                };
            }
            case "list_vpas": {
                const namespace = args.namespace;
                try {
                    const vpas = await customObjectsApi.listNamespacedCustomObject({ group: "autoscaling.k8s.io", version: "v1", namespace, plural: "verticalpodautoscalers" });
                    const items = (vpas.items || []).map((v) => ({
            ...
            case "get_vpa_recommendation": {
                ...
                try {
                    const vpa = await customObjectsApi.getNamespacedCustomObject({ group: "autoscaling.k8s.io", version: "v1", namespace, plural: "verticalpodautoscalers", name: vpaName });
            ...
            case "list_storage_classes": {
                const scs = await customObjectsApi.listClusterCustomObject({ group: "storage.k8s.io", version: "v1", plural: "storageclasses" });
                const items = (scs.items || []).map((s) => ({
            ...
            case "get_storage_class": {
                ...
                const sc = await customObjectsApi.getClusterCustomObject({ group: "storage.k8s.io", version: "v1", plural: "storageclasses", name: storageClassName });
                return {
                    content: [{
                            type: "text",
                            text: safeJson({
            ...
            case "get_addon_health": {
                ...
                const findDep = (names) => deployments.items.find((d) => names.includes(d.metadata?.name || ""));
                const findDs = (names) => daemonsets.items.find((d) => names.includes(d.metadata?.name || ""));
            ...
            case "list_limit_ranges": {
                ...
                const items = (limits.items || []).map((l) => ({
                    name: l.metadata?.name,
                    limits: (l.spec?.limits || []).map((x) => ({
                        type: x.type,

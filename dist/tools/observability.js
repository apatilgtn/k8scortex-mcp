...
export async function handleObservabilityTool(name, args) {
    ...
    try {
        switch (name) {
            ...
            case "list_warning_events": {
                ...
                const warnings = res.items.map((e) => ({
                    reason: e.reason,
                    message: e.message,
            ...
            case "get_node_pressure": {
                ...
                const conditions = res.status?.conditions?.filter((c) => c.type.includes('Pressure'));

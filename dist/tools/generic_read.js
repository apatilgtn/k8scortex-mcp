...
export async function handleGenericReadTool(name, args) {
    ...
    try {
        if (name === "list_k8s_resources") {
            if (!apiGroup) {
                switch (resource) {
                    case "persistentvolumeclaims": {
                        if (!namespace)
                            throw new Error("namespace is required for persistentvolumeclaims");
            ...
            if (namespaced) {
                if (!namespace)
                    throw new Error("namespace is required when namespaced=true");

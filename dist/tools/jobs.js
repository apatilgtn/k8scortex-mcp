...
export async function handleJobTool(name, args) {
        ...
        switch (name) {
            case "list_jobs": {
                ...
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify(res.items.map((j) => ({
                                name: j.metadata?.name,
                                status: j.status?.conditions?.find((c) => c.type === 'Complete' && c.status === 'True') ? 'Complete' :
                                    j.status?.conditions?.find((c) => c.type === 'Failed' && c.status === 'True') ? 'Failed' : 'Active',
            ...
            case "list_cronjobs": {
                ...
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify(res.items.map((cj) => ({
